...
import { LayoutDashboard, FolderOpen, User, Bell, Lock, LogOut, ExternalLink } from 'lucide-react';
    ...
    <div className="flex min-h-screen bg-white font-sans text-gray-800">
      ...
      <aside className="w-64 bg-[#fcfcfc] border-r border-gray-100 flex flex-col fixed h-full z-10 hidden md:flex">
        <div className="p-8">
          ...
          {user && (
            <p className="text-gray-500 text-xs mt-3 truncate">👋 {user.name || user.username}</p>
          )}
        </div>
          ...
          {isAdmin && (
            <>
              <div onClick={() => navigate('/admin')}>
                <SidebarItem icon={<LayoutDashboard size={20} />} text="Article management" active={isActive('/admin') || isActive('/create-post')} />
              </div>
        ...
        <div className="p-4 mt-auto border-t border-gray-100">
          <button onClick={() => navigate('/')}
            className="flex items-center gap-3 text-gray-500 hover:text-black p-3 transition-colors w-full text-sm font-medium">
            <ExternalLink size={18} /> View website
          </button>
          <button onClick={handleLogout}
            className="flex items-center gap-3 text-gray-500 hover:text-red-600 p-3 transition-colors w-full text-sm font-medium">
...
function SidebarItem({ icon, text, active = false }: { icon: any; text: string; active?: boolean }) {
  return (
    <div className={`flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-all mb-1
